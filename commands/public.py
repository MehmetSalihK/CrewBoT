        ...
        for s, sv in pv['secondaries'].items():
            if s == vc.id:
                if 'priv' not in sv or not sv['priv']:
                    return False, ("Votre chaîne est déjà publique."
                                   "Utilisez `{}private` pour le rendre privé à la place.".format(ctx['print_prefix']))
                try:
                ...
                try:
                    ...
                    return False, "You channel is now public, but I'm not allowed to delete your **⇩ Rejoignez ** channel."
                return True, "Votre chaîne est désormais publique!"
