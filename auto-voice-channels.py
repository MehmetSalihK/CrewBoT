    ...
    if client.is_ready():
        ...
        for mid in votekicks:
            ...
            if in_favor >= vk['required_votes']:
                ...
                log("Coup de pied {} de {} ({}/{})".format(vk['offender'].display_name,
                ...
                try:
                    ...
                    await vk['message'].edit(
                        content="‼ **Votekick** échoué - Une erreur `{}` a été rencontrée.".format(type(e).__name__))
                    continue
                banned = True
                try:
                ...
                await vk['message'].edit(content=(
                    "‼ **Votekick** ‼\n"
                    "{} était **kick** de {} canal {}.{}".format(
                        vk['offender'].mention, vk['initiator'].mention,
                        (", mais n'a pas pu être banni de la chaîne car je n'ai pas l'autorisation *Gérer les rôles*."
                         if not banned else ""),
                        ("\nRaison: **{}**".format(vk['reason']) if vk['reason'] else ""))
                ))
                await func.server_log(
                    guild,
                    "👢 {} (`{}`) a été **kick** de {}' canal.".format(
                ...
                await vk['message'].edit(content="‼ **Votekick** expiré: votes insuffisants reçus "
                                                 "({0}/{1}), obligatoire: {2}/{1}.".format(in_favor,
                                                                                        len(vk['participants']) + 1,
                                                                                        vk['required_votes']))
        for mid in to_remove:
            log("SUPPRIMER VOTEKICK: {} {} len:{} keys:{}".format(
                mid,
                type(mid),
                len(cfg.VOTEKICKS),
    ...
    for pc in priv_channels:
        ...
        if 'request_time' in pcv and time() - pcv['request_time'] > 120:
            ...
            await pcv['text_channel'].send(
                ":warning: {} For some reason I was unable to create your \"⇩ Rejoignez\" channel, please try again later. "
        ...
        for p, pv in settings['auto_channels'].items():
            for s, sv in pv['secondaries'].items():
                if 'priv' in sv and 'jc' not in sv:
                    ...
                    try:
                        jc = await guild.create_voice_channel("⇩ Rejoignez {}".format(creator),  # TODO creator can change
