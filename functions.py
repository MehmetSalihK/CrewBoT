        ...
        for s, sv in pv['secondaries'].items():
            if s == cid:
                ...
                try:
                    ...
                    await jc.edit(name="⇩ Rejoignez {}".format(creator.display_name))
...
async def create_secondary(guild, primary, creator, private=False):
    ...
    if user_request_is_locked(creator):
        ...
        log("{} ({}) tried creating a channel where I don't have permissions".format(creator.display_name,
                                                                                     creator.id), guild)
        msg = "{} ❌ Vous avez essayé de créer une chaîne où je n'ai pas les bonnes autorisations.".format(creator.mention)
        server_contact = guild.get_member(settings['server_contact'])
        msg += "\n\nVeuillez vous assurer que j'ai les autorisations suivantes"
        if primary.category:
            msg += " in the \"{}\" category:\n".format(primary.category.name)
        else:
            msg += ":\n"
        msg += "- **Gérer la chaîne**\n"
        msg += "- **Lire les chaînes de texte et voir les chaînes vocales**\n"
        msg += "- **Envoyer des messages**\n"
        msg += "- **Connecter ** *(sous les autorisations des canaux vocaux)*\n"
        msg += "- **Déplacer des membres**\n\n"
        if server_contact is not None and server_contact != creator:
            msg += "Si vous n'êtes pas administrateur / gestionnaire de ce serveur, "
            msg += "{} pourrait être en mesure de vous aider.\n".format(server_contact.mention)
        msg += "Si vous avez besoin d'aide, vous pouvez rejoindre mon serveur d'assistance: <https://discord.gg/NGWef32>.\n\n"
        msg += "Ce message se répétera toutes les 5 minutes si le problème n'est pas résolu. "
        msg += "Pour arrêter cela, corrigez le problème ou quittez le canal vocal à l'origine du problème."
    ...
    if 'text_channels' in settings and settings['text_channels'] and is_gold(guild):
        ...
        tc = await guild.create_text_channel(
            utils.nice_cname(settings['text_channel_name']) if 'text_channel_name' in settings else "Text",
            category=primary.category,
            overwrites=overwrites,
